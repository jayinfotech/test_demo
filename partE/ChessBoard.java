public class ChessBoard {
		...
		for (int i = 0; i < 8; i++) {
			...
			for (int j = 0; j < 8; j++) {
				...
				if (locations[i][j].getPiece() != null) {
					//System.out.print(locations[i][j].getPiece().toString()+""+locations[i][j].getPiece().getColour());
	...
	public boolean freeDiagonalPath(Location from, Location to) throws IllegalChessMoveException {
		...
		if (from.getCol() < to.getCol()) {
			...
			while (col < to.getCol()) {
				...
				if (locations[row][col].getPiece() == null) {
					...
					throw new IllegalChessMoveException(IllegalChessMoveException.OBSTACLE);
				}
	...
	public boolean freeAntidiagonalPath(Location from, Location to) throws IllegalChessMoveException {
		if (from.getCol() < to.getCol()) {
	...
	public boolean freeHorizontalPath(Location from, Location to) throws IllegalChessMoveException  {
	...
	public boolean freeVerticalPath(Location from, Location to) throws IllegalChessMoveException  {
